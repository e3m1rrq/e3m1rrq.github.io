...
const firebaseConfig = {
  ...
  storageBucket: "cafe-90be8.appspot.com", // FIX: Firebase Storage bucket should end with .appspot.com
...
// Basic HTML escape to avoid broken attributes if name contains quotes/angle brackets
...
async function handleAdminLogin(){
  ...
  const stored = adminConfig.adminPassword || '';
  const ok = stored ? await verifyPassword(pwd, stored) : (pwd === '');
  if (ok){
...
function loadAdminPanelData(){
  ...
  qs('add-product-btn').addEventListener('click', addProductFromInputs);
...
function addProductFromInputs(){
  ...
  if (!name || !Number.isFinite(price) || !Number.isFinite(stock)){
    alert('Lütfen geçerli bir ad, fiyat ve stok girin.'); return;
  }
  const id = 'prod' + Date.now();
  db.ref('products/' + id).set({ name, price, stock })
    .then(()=>{ nameEl.value=''; priceEl.value=''; stockEl.value=''; })
    .catch(e => alert('Ürün eklenemedi: ' + e.message));
}
...
function renderProductManagement(products){
  ...
  Object.keys(products).forEach(productId=>{
    ...
    div.innerHTML = `
      <div>
        <input type="text" value="${escapeHTML(p.name ?? '')}" data-field="name" placeholder="Ad">
...
function calculateDailyAnalysis(dailyBook){
  ...
  expensesEl.textContent = money(totalExpenses);
  profitEl.textContent = money(totalRevenue - totalExpenses);
}
